...
public class WorkForceController {
    ...
    public void setMainApp(MainApp mainApp) {
        ...
        Cursor cursor = mainApp.manager.getAllData("workforce");
        ...
        while(iterator.hasNext()){
            ...
            projectList.add(new ProjectType(
                    ...
                    hashMap.get("montant").toString(),"","",hashMap.get("id").toString(), hashMap.get("specialisation").toString()
            ));
        }
    }
                ...
                if(main_libelle.getText().isEmpty() || main_taux.getText().isEmpty()){
                    ...
                    try{
                        mainApp.mapObject = ConnectionManager.getR()
                                .hashMap("name",main_libelle.getText())
                                .with("montant",main_taux.getText())
                                .with("specialisation",main_qualification.getValue().toString());

                        if (id.getText().isEmpty()){
                            mainApp.manager.SaveData(mainApp.mapObject,"workforce");
                            currentsaveId = mainApp.manager.getCurrentsaveId();
                        }else{

                            mainApp.manager.UpdateData(mainApp.mapObject,"workforce",id.getText());
                        ...
                        projectList.add(new ProjectType(
                                main_libelle.getText(), main_taux.getText(),
                                main_taux.getText(),main_libelle.getText(),
                                currentsaveId,main_qualification.getValue().toString()
                        ));

                        main_taux.setText("");
                        main_libelle.setText("");
                    }catch (NullPointerException as){
                        new WindowZone("Sauvegarde Impossible","Vous devez definir la qualification","Aucune Qualification",true,stage);
                    }
            ...
            p.setOnMouseClicked(event -> {
                if (event.getClickCount() == 2 && !p.isEmpty()){
                    ...
                    try {
                        ...
                        main_taux.setText(projectType.getQualification());
        ...
        delete.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                if (alerts.getResult().getButtonData().isDefaultButton()){
                    mainApp.manager.deleteData("workforce",selected.getId());
                    int index = tableView.selectionModelProperty().getValue().getSelectedIndex();
                    tableView.getItems().remove(index);
                }
            }
